...
i8 have_xmas(char grid[MAX_N_ROWS][MAX_N_COLS], i32 row, i32 col, i32 n_rows, i32 n_cols) {
    i32 count = 0;
    if (
        grid_eq('X', grid, row, col + 0, n_rows, n_cols) &&
        grid_eq('M', grid, row, col + 1, n_rows, n_cols) &&
        grid_eq('A', grid, row, col + 2, n_rows, n_cols) &&
        grid_eq('S', grid, row, col + 3, n_rows, n_cols)
    ) {
        printf("col+;");
        count += 1;
    }
    if (
        grid_eq('X', grid, row + 0, col, n_rows, n_cols) &&
        grid_eq('M', grid, row + 1, col, n_rows, n_cols) &&
        grid_eq('A', grid, row + 2, col, n_rows, n_cols) &&
        grid_eq('S', grid, row + 3, col, n_rows, n_cols)
    ) {
        printf("row+;");
        count += 1;
    }
    ...
    if (
        grid_eq('X', grid, row + 0, col + 0, n_rows, n_cols) &&
        grid_eq('M', grid, row + 1, col + 1, n_rows, n_cols) &&
        grid_eq('A', grid, row + 2, col + 2, n_rows, n_cols) &&
        grid_eq('S', grid, row + 3, col + 3, n_rows, n_cols)
    ) {
        printf("row+col+;");
        count += 1;
    }
